// Bridge logger: structured JSONL to logs/bridge.log with 5MB rotation, keep 5.
// Mirrors the agent logger. PROTOCOL.md §6.1 + §6.3.
...
export class BridgeLogger {
  ...
  constructor(opts: { device: string; logDir: string }) {
    ...
    this.filePath = path.join(opts.logDir, "bridge.log");
    try {
  ...
  debug(event: string, message: string, extra?: Partial<Parameters<BridgeLogger["log"]>[0]>) {
    this.log({ level: "debug", event, message, ...(extra ?? {}) });
  }
  info(event: string, message: string, extra?: Partial<Parameters<BridgeLogger["log"]>[0]>) {
    this.log({ level: "info", event, message, ...(extra ?? {}) });
  }
  warn(event: string, message: string, extra?: Partial<Parameters<BridgeLogger["log"]>[0]>) {
    this.log({ level: "warn", event, message, ...(extra ?? {}) });
  }
  error(event: string, message: string, extra?: Partial<Parameters<BridgeLogger["log"]>[0]>) {
